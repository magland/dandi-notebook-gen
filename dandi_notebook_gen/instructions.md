...
4. Do exploratory research on the contents of the Dandiset by creating and executing python scripts in a tmp_scripts subdirectory to generate text output and plots.
  ...
  - After executing each script, if you created plots, review the plots using the read_image tool to be able to gain information about them. Each call to read_image should be followed by a call to the reflect tool to provide information about the plot as relevant to the task. If the plot is not informative and useful (for example containing no data) or does not convey the expected information, you should note that in the reflection, and then you may need to adjust the script and re-run it or exclude that plot from the final notebook.
